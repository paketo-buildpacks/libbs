...
func NewApplication(additionalMetadata map[string]interface{}, arguments []string, artifactResolver ArtifactResolver, cache Cache, command string, plan *libcnb.BuildpackPlan, applicationPath string) (Application, error) {
	a :=  Application{
...
func (a Application) ExpectedMetadata() (map[string]interface{}, error) {
	...
	for k, _ := range a.AdditionalMetadata {
		metadata[k] = a.AdditionalMetadata[k]
	}
	a.expectedMetadata = metadata
	return metadata, nil
}
...
func (a Application) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	metadata, err := a.ExpectedMetadata()
	if err != nil {
		return libcnb.Layer{}, fmt.Errorf("error building metadata\n%w", err)
	}
